...
const tryIpApi = async (): Promise<GeoLocation | null> => {
  try {
    const json = await fetchJson("http://ip-api.com/json/?fields=city,country,lat,lon,timezone");
...
export const guessCityCountry = async (query: string): Promise<CityCountryGuess | null> => {
  const trimmedQuery = normalizeCityQuery(query);
  if (!trimmedQuery) {
    return null;
  }
